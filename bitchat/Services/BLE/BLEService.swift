...
/// - Emits events exclusively via `MeshChatDelegate` for UI.
...
final class BLEService: NSObject {
    ...
    private var pendingDirectedRelays: [PeerID: [String: (packet: MeshChatPacket, enqueuedAt: Date)]] = [:]
    ...
    weak var delegate: MeshChatDelegate?
    ...
    func stopServices() {
        ...
        let leavePacket = MeshChatPacket(
    ...
    func sendFileBroadcast(_ filePacket: MeshChatFilePacket, transferId: String) {
        messageQueue.async { [weak self] in
            guard let self = self else { return }
    ...
    func sendFilePrivate(_ filePacket: MeshChatFilePacket, to peerID: PeerID, transferId: String) {
        messageQueue.async { [weak self] in
            guard let self = self else { return }
            guard let payload = filePacket.encode() else {
    ...
    private func validatePacket(_ packet: MeshChatPacket, from peerID: PeerID) -> Bool {
    ...
    private func broadcastPacket(_ packet: MeshChatPacket, transferId: String? = nil) {
        // Apply route if recipient exists (centralized route application)
        let packetToSend: MeshChatPacket
    ...
    private func sendEncrypted(_ packet: MeshChatPacket, data: Data, pad: Bool) {
    ...
    private func sendGenericBroadcast(_ packet: MeshChatPacket, data: Data, pad: Bool) {
    ...
    private func sendOnAllLinks(packet: MeshChatPacket, data: Data, pad: Bool, directedOnlyPeer: PeerID?) {
    ...
    private func sendPacketDirected(_ packet: MeshChatPacket, to peerID: PeerID) {
    ...
    private func spoolDirectedPacket(_ packet: MeshChatPacket, recipientPeerID: PeerID) {
    ...
    private func flushDirectedSpool() {
        ...
        let toSend: [(String, MeshChatPacket)] = collectionsQueue.sync(flags: .barrier) {
            var out: [(String, MeshChatPacket)] = []
            let now = Date()
    ...
    private func handleFileTransfer(_ packet: MeshChatPacket, from peerID: PeerID) {
        ...
        guard let filePacket = MeshChatFilePacket.decode(packet.payload) else {
    ...
    private func handleLeave(_ packet: MeshChatPacket, from peerID: PeerID) {
...
extension BLEService: GossipSyncManager.Delegate {
    func sendPacket(_ packet: MeshChatPacket) {
        broadcastPacket(packet)
    }

    func sendPacket(to peerID: PeerID, packet: MeshChatPacket) {
        sendPacketDirected(packet, to: peerID)
    }

    func signPacketForBroadcast(_ packet: MeshChatPacket) -> MeshChatPacket {
...
extension BLEService {
    func _test_handlePacket(_ packet: MeshChatPacket, fromPeerID: PeerID) {
...
extension BLEService: CBPeripheralDelegate {
    ...
    private func processNotificationPacket(_ packet: MeshChatPacket, from peripheral: CBPeripheral, peripheralUUID: String) {
        let senderID = PeerID(hexData: packet.senderID)
...
extension BLEService {
    ...
    private func applyRouteIfAvailable(_ packet: MeshChatPacket, to recipient: PeerID) -> MeshChatPacket {
        ...
        let routedPacket = MeshChatPacket(
            type: packet.type,
    ...
    private func forwardAlongRouteIfNeeded(_ packet: MeshChatPacket) -> Bool {
    ...
    private func makeMessageID(for packet: MeshChatPacket) -> String {
    ...
    private func priority(for packet: MeshChatPacket, data: Data) -> OutboundPriority {
    ...
    private func sendFragmentedPacket(_ packet: MeshChatPacket, pad: Bool, maxChunk: Int? = nil, directedOnlyPeer: PeerID? = nil, transferId: String? = nil) {
    ...
    private func startFragmentedPacket(_ context: PendingFragmentTransfer) {
        ...
        for (index, fragment) in fragments.enumerated() {
            ...
            let fragmentPacket = MeshChatPacket(
    ...
    private func handleFragment(_ packet: MeshChatPacket, from peerID: PeerID) {
        if DispatchQueue.getSpecific(key: messageQueueKey) != nil {
    ...
    private func _handleFragment(_ packet: MeshChatPacket, from peerID: PeerID) {
    ...
    private func handleReceivedPacket(_ packet: MeshChatPacket, from peerID: PeerID) {
        // Call directly if already on messageQueue, otherwise dispatch
        if DispatchQueue.getSpecific(key: messageQueueKey) == nil {
            messageQueue.async { [weak self] in
    ...
    private func handleAnnounce(_ packet: MeshChatPacket, from peerID: PeerID) {
    ...
    private func handleRequestSync(_ packet: MeshChatPacket, from peerID: PeerID) {
    ...
    private func handleMessage(_ packet: MeshChatPacket, from peerID: PeerID) {
    ...
    private func handleNoiseHandshake(_ packet: MeshChatPacket, from peerID: PeerID) {
        ...
        if PeerID(hexData: packet.recipientID) == myPeerID {
            ...
            do {
                if let response = try noiseService.processHandshakeMessage(from: peerID, message: packet.payload) {
                    // Send response
                    let responsePacket = MeshChatPacket(
                        type: MessageType.noiseHandshake.rawValue,
                        senderID: myPeerIDData,
                        recipientID: Data(hexString: peerID.id),
                        timestamp: UInt64(Date().timeIntervalSince1970 * 1000),
    ...
    private func handleNoiseEncrypted(_ packet: MeshChatPacket, from peerID: PeerID) {
    ...
    private func performCleanup() {
        ...
        collectionsQueue.async(flags: .barrier) { [weak self] in
            ...
            if !self.pendingDirectedRelays.isEmpty {
                var cleaned: [PeerID: [String: (packet: MeshChatPacket, enqueuedAt: Date)]] = [:]
