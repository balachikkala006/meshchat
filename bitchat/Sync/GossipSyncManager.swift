...
final class GossipSyncManager {
    ...
    private struct PacketStore {
        private(set) var packets: [String: MeshChatPacket] = [:]
        private(set) var order: [String] = []

        mutating func insert(idHex: String, packet: MeshChatPacket, capacity: Int) {
        ...
        func allPackets(isFresh: (MeshChatPacket) -> Bool) -> [MeshChatPacket] {
        ...
        mutating func remove(where shouldRemove: (MeshChatPacket) -> Bool) {
        ...
        mutating func removeExpired(isFresh: (MeshChatPacket) -> Bool) {
            remove { !isFresh($0) }
        }
    }
    ...
    private var latestAnnouncementByPeer: [PeerID: (id: String, packet: MeshChatPacket)] = [:]
    ...
    func onPublicPacketSeen(_ packet: MeshChatPacket) {
    ...
    private func isPacketFresh(_ packet: MeshChatPacket) -> Bool {
    ...
    private func isAnnouncementFresh(_ packet: MeshChatPacket) -> Bool {
    ...
    private func _onPublicPacketSeen(_ packet: MeshChatPacket) {
